...
RestTest.prototype = {
	...
	useParams: function(type, key, value){
		if(typeof type === 'object'){
			this.params = _.chain(type)
				.pick(['path', 'header', 'query', 'body', 'form', 'file'])
	...
	useForm: function(key, value){
		return this.useParams('form', key, value);
	},
	...
	useFile: function(key, value){
		return this.useParams('file', key, value);
	},
			...
			return function(){
				...
				if(self.children.length > 0){
					...
					it(self.description.value, function(done){
						...
						if(rt.requestHeaders){
							request.set(rt.requestHeaders);
						}
						...
						if(rt.params.form && !rt.params.form.__removed){
							var queryArgs = _.reduce(rt.params.form, function(a, p){
								if(p.__removed){return a;}
								a[p.key] = p.value;
								return a;
							}, {});
							debug('Configuring form', queryArgs);
							request.type('form');
							request.send(queryArgs);
						}
						...
						if(rt.params.file && !rt.params.file.__removed){
							debug('Configuring file');
							_.forEach(rt.params.file, function(f){
								if(!f.__removed){
									request.attach(f.key, f.value);
								}
							});
						}
	...
	_authentication:{
		...
		sessionCookie: function(app, authScheme, credentials, restTest, done){
			q()
			.then(function(){
				if(credentials.cookie){
					debug('cookie retrieved from cache ', credentials.cookie);
					return credentials.cookie;
				}
				else{
					debug('Logging in and Retrieving cookie');
					
					var loginRequest = supertest(app)
						.post(authScheme.login)
						.send(credentials)
						.set('Accept', 'application/json')
						.expect(200);
					return q.ninvoke(loginRequest, 'end')
					.then(function(res){
						debug('login headers', res.header);
						expect(res.header).to.have.property('set-cookie');
						var cookie = res.header['set-cookie'];
						debug('caching cookie for %s', credentials.username, cookie);
			...
			.then(function(cookie){
				restTest.requestHeaders = restTest.requestHeaders || {};
				restTest.requestHeaders.Cookie = cookie;
				debug('Set Request header Cookie %s', cookie);
				done();
			})
			.fail(function(err){
				debug('An Error occured whilst configuring authorization', err);
