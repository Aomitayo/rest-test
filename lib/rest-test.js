...
RestTest.prototype = {
	...
	/**
	 * Specifies the authorization scheme to be used
	 * @param  {[type]} description   [description]
	 * @param  {[type]} authorization [description]
	 * @return {Object}               The current test case.
	 */
	useAuthorization: function(description, authorization){
		if(!authorization){
			authorization = description;
			description = Date.now();
		}
		this.authorization = {description: description, value: authorization};

		return this;
	},

	/**
	 * Specifies that no authorization scheme should be used for the test
	 * @return {Object}               The current test case.
	 */
	withoutAuthorization: function(){
		this.authorization = {__removed:true};
		return this;
	},
	 ...
	 * @return {object}                     The current test case.
	 */
	useCredentials: function(description, credentials, type){
		...
		this.credentials[type] = {description: description, value: credentials, type: type};
		...
		if(groupKeys.length > 0){
			...
			return function(){
				...
				beforeEach(function(){
					...
					debug('Merging Credentials for ' + self.description.value, self.credentials);

					if(self.credentials && self.credentials.__removed){
						restTest.credentials = null;
					}
					else{
						restTest.credentials = _.assign(restTest.credentials || {}, self.credentials || {});
					}
				...
				if(self.children.length > 0){
					...
					beforeEach(function(done){
						...
						debug('Credentials for ' + self.description.value, rt.credentials);
						
						if(rt.authorization && rt.authorization.type === 'oauth2' && rt.credentials && rt.credentials.user && rt.credentials.client && !rt.credentials.__removed){
							debug('Configuring Authorization');

							if(rt.authorization.grantTypes.password){

								q()
								.then(function(){
									var userCredentials = rt.credentials.user.value;
									var clientCredentials = rt.credentials.client.value;

									if(userCredentials.accessToken){
										debug('accessToken retrieved from cache ', userCredentials.accessToken);
										return userCredentials.accessToken;
									}
									else{
										debug('Retrieving accessToken');
										var tokenEndpoint = rt.authorization.grantTypes.password.tokenEndpoint;
										...
										var tokenRequest = supertest(rt.app)
										...
										.then(function(res){
											...
											debug('caching accessToken for %s', userCredentials.username, res.body[tokenEndpoint.tokenName]);
											userCredentials.accessToken = res.body[tokenEndpoint.tokenName];
											return userCredentials.accessToken;
										});
									}
								...
								.then(function(accessToken){
									rt.authorizationHeader = 'Bearer ' + accessToken;
									debug('Set authorization header %s', rt.authorizationHeader);
									done();
								})
								.fail(function(err){
								...
								debug('no recognized authorization scheme');
								done();
							}
						}
						else{
							debug('Authorization or credentials not specified');
							done();
						}
					});
